...
class commentaireModele {
	...
	public function getCommentaireS() {
		...
		if ($this->IDC) {
			$result = $this->IDC->query ( "SELECT * from commentaire c inner join users u on u.IDU = c.IDU;" );
			return $result;
		}
	}
	...
	public function getCommentairesIdjv($idJ) {
		...
		if ($this->IDC) {
			$result = $this->IDC->query ( "SELECT * from commentaire c inner join users u on u.IDU = c.IDU where idjv =".$idJ.";" );
			return $result;
		}
	}
